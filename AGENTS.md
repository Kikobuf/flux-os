...
All commands start with `flux-`. Prefixes indicate purpose.

The authoritative command group list lives in `bin/flux` in `GROUP_DESCRIPTIONS`. Keep `GROUP_DESCRIPTIONS` updated when adding a new command prefix.
...
Commands in `bin/` can declare CLI metadata in comments near the top of the file. `bin/flux` scans the first 80 lines, and tests expect command metadata to remain valid.

Supported metadata keys:

- `# flux:summary=...` - short help text
- `# flux:group=...` - command group when it differs from the filename-derived prefix
- `# flux:name=...` - command name within the group
- `# flux:args=...` - usage arguments
- `# flux:examples=...` - examples separated with ` | `
- `# flux:alias=...` / `# flux:aliases=...` - alternate routes
- `# flux:hidden=true` - hide from default command listings
- `# flux:requires-sudo=true` - mark commands that require sudo
...
```bash
# flux:summary=Take a screenshot
# flux:group=capture
# flux:args=[smart|region|windows|fullscreen] [slurp|copy]
# flux:examples=flux screenshot | flux capture screenshot region
# flux:aliases=flux screenshot
```
...
- leaf scripts are sourced by `run_logged $FLUX_INSTALL/path/to/script.sh`
- avoid `exit` in sourced install scripts unless intentionally aborting the install
- use `$FLUX_INSTALL` and `$FLUX_PATH` instead of hard-coded Flux paths
...
- `flux-cmd-missing` / `flux-cmd-present` - check for commands
- `flux-pkg-missing` / `flux-pkg-present` - check for packages
- `flux-pkg-add` - install packages (handles both pacman and AUR)
- `flux-notification-send` - send desktop notifications; do not call `notify-send` directly
- `flux-hw-asus-rog` - detect ASUS ROG hardware (and similar `hw-*` commands)
...
When making visual changes, such as Waybar styles or desktop appearance, always take and analyze a screenshot after applying the change to verify the result. Use `flux capture screenshot fullscreen save` for fullscreen screenshots.
...
```bash
flux-refresh-config hypr/hyprlock.conf
```

This copies `~/.local/share/flux/config/hypr/hyprlock.conf` to `~/.config/hypr/hyprlock.conf`.

# Migrations

To create a new migration, run `flux-dev-add-migration --no-edit`. This creates a migration file named after the unix timestamp of the last commit.

New migration format:
- File permissions must be `0644` (`-rw-r--r--`); migrations are sourced, not executed directly
- No shebang line
- Start with an `echo` describing what the migration does
- Use `$FLUX_PATH` to reference the flux directory
- Prefer helper commands such as `flux-cmd-present`, `flux-cmd-missing`, `flux-pkg-present`, and `flux-pkg-missing`

Some older migrations predate these rules. Do not copy older migrations that start with shebangs, omit the leading `echo`, or hard-code `~/.local/share/flux`.
...
if flux-cmd-missing fprintd-list || ! fprintd-list "$USER" 2>/dev/null | grep -q "finger"; then
