--
-- Dynamic Flux Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the flux-shipped Plymouth via
-- flux-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs flux-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "fluxunlocks"
NamePretty = "Flux Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/flux/themes"
  local flux_path = os.getenv("FLUX_PATH") or ""
  local default_themes_dir = flux_path .. "/themes"
  local default_preview = flux_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "flux-launch-floating-terminal-with-presentation "
                .. shell_escape("flux-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "flux-launch-floating-terminal-with-presentation "
        .. shell_escape("flux-plymouth-reset"),
    },
  }
