echo "Migrate Hyprland user config from Flux hyprlang files to Lua files"

hypr_config_dir="$HOME/.config/hypr"
flux_hypr_config_dir="$FLUX_PATH/config/hypr"
toggle_state_dir="$HOME/.local/state/flux/toggles/hypr"
toggle_defaults_dir="$FLUX_PATH/default/hypr/toggles"
timestamp=$(date +%s)
...
for lua_config in "$flux_hypr_config_dir"/*.lua "$flux_hypr_config_dir"/.luarc.json; do
...
if [[ -f $sddm_hypr_conf || -f $sddm_wayland_conf ]]; then
  if [[ -f $FLUX_PATH/default/sddm/hyprland.lua ]]; then
    sudo mkdir -p /usr/share/sddm
    sudo cp "$FLUX_PATH/default/sddm/hyprland.lua" "$sddm_hypr_lua"
  fi
