...
if $NEEDS_MIGRATION; then
  ...
  flux-pkg-drop walker-bin walker-bin-debug

  flux-pkg-add "${PACKAGES[@]}"

  source $FLUX_PATH/install/config/walker-elephant.sh

  rm -rf ~/.config/walker/themes

  flux-refresh-config walker/config.toml
  flux-refresh-config elephant/calc.toml
  flux-refresh-config elephant/desktopapplications.toml
fi
