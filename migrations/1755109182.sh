...
# Users can still opt-in to Cloudflare DNS using: flux-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-flux-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-flux-dns.network
  ...
  echo "To use Cloudflare DNS, run: flux-setup-dns Cloudflare"
fi