...
# Move user-added backgrounds from Flux theme folders to user config
FLUX_DIR="$HOME/.local/share/flux"
USER_BACKGROUNDS_DIR="$HOME/.config/flux/backgrounds"

if [[ -d $FLUX_DIR/themes ]]; then
  cd "$FLUX_DIR"

  # Get list of git-tracked background files (relative to flux dir)
...
THEMES_DIR="$HOME/.config/flux/themes"
CURRENT_THEME_LINK="$HOME/.config/flux/current/theme"
...
elif [[ -f $HOME/.config/flux/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/flux/current/theme.name")
fi

# Remove all symlinks from ~/.config/flux/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  flux-theme-set "$CURRENT_THEME_NAME"
else
  # Backup to ensure a theme is set if we can't deduce the name
  flux-theme-set "Tokyo Night"
fi
